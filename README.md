...
- 初始化协程时，调用 co_alloc_sharestack 从共享栈中指定一块内存作为栈的私有栈空间，后续该co执行时都是用这一块共享栈空间。
...
- C++ 有引用和指针，当协程栈通过 malloc & memcpy 重分配空间时，指向原栈空间的指针和引用都会失效，引起未定义行为。libco为每个co分配的共享栈空间不较大，而且由于是多个co共享一块空间，只有公用同一个空间的co在切换时需要进行stack的拷贝，共享栈方式节省了内存，避免大内存的频繁分配，但stack拷贝有开销。
