...
可以看到随线程数增加，吞吐量明显提高。说明无锁的实现大大提高了日志的写入速度。但这样的设计瓶颈落在了缓冲区大小和读线程上，目前设置的是 1M。可以看到当消息大小为 1KB 时，吞吐量下降很快。这是由于前端写入太快，日志线程无法及时消费，导致 CPU 轮询的时间增加，且此时 CPU 负载为100%。但对于普通的小消息日志，在多线程下也有较好的性能表现。
